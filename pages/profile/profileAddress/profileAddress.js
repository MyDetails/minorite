...
const profileAddress = {
    ...
    template: `
            <div class="content">
                ...
                <div class="profileAddress-content main">
                    ...
                    <div class="profileAddress-box">
                        <Tabs :animated="false" type="card">
                            <TabPane label="我的地址">
                                <ul class="address-list">
                                  <li v-for="item in address_list" :key="item.id">
                                    <div class="address-item" :class="item.def ? 'def_bg' : ''">
                                        <p><span>我的</span>收货地址：</p>
                                        <p>收&nbsp;&nbsp;货&nbsp;&nbsp;人：<span>{{item.name}}</span></p>
                                        <p>联系电话：<span>{{item.mobile}}</span></p>
                                        <!-- <span>所&nbsp;&nbsp;在&nbsp;&nbsp;地：<span>北京市 北京市 朝阳区</span></span> -->
                                        <p style="display:flex;"><span>地&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;址：</span><span>{{item.address}}</span></p>
                                        <div class="moren-address">
                                            <input class="moren-address-btn" type="button" value="设为默认地址" @click="defAddress(item.id,item.address,item.mobile,item.name)">
                                        </div>
                                        <div class="del-address">
                                            <Poptip confirm title="确认删除地址吗？" @on-ok="ok(item.id)" @on-cancel="cancel"> 
                            ...
                            <TabPane label="新增地址">
                                <div class="editor-address-box">
                                    <Form class="address-form" ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
                                        <FormItem class="select-address-input" label="区域选择：" :label-width="100" prop="text">
                                            <div id="example">
                                                <select id="prov" v-model="prov" style="float: left;margin-right:15px;background:#f7f9f3;text-indent:1.5em;padding:0 2%;width:80px;">
                                                    <option v-for="(option,i) in arr" :key="i" :value="option.name" bindtap="provnm(option.name)" style="width: 140px;text-align: center;">
                                                        {{ option.name }}
                                                    </option>
                                                </select>
                                                <select id="city" v-model="city" style="float: left;margin-right:15px;background:#f7f9f3;text-indent:1.5em;padding:0 2%;width:120px;">
                                                    <option v-for="(option,i) in cityArr" :key="i" :value="option.name" bindtap="citynm(option.name)" style="width: 140px;text-align: center;">
                                                        {{ option.name }}
                                                    </option>
                                                </select>
                                                <select id="district" v-model="district" v-if="district" style="float: left;background:#f7f9f3;text-indent:1em;padding:0 2%;width:120px;">
                                                    <option v-for="(option,i) in districtArr" :key="i" :value="option.name" bindtap="districtnm(option.name)" style="width: 140px;text-align: center;">
                                                        {{ option.name }}
                                                    </option>
                                                </select>
                                            </div>
                                        </FormItem>
                                        <FormItem class="address-input" label="详细地址：" :label-width="100" prop="address">
                                            <Input v-model="formValidate.address" placeholder=""></Input>
                                        </FormItem>
                                        <FormItem class="abbreviation-input" label="邮政编码：" :label-width="100" prop="abbreviation">
                                            <Input v-model="formValidate.abbreviation" placeholder=""></Input>
                                        </FormItem>
                                        <FormItem class="name-input" label="收货人姓名：" :label-width="120" prop="name">
                                            <Input v-model="formValidate.name" placeholder=""></Input>
                                        </FormItem>
                                        <FormItem class="phone-input" label="手机号码：" :label-width="100" prop="phone">
                                            <Input v-model="formValidate.phone" placeholder=""></Input>
                                        </FormItem>
                                        <FormItem class="comfirm-address">
                                            <Button class="add-address-btn address-btn" type="primary" @click="handleSubmit('formValidate')">确认</Button>
                                        </FormItem>
                                    </Form>
                                </div>
                            </TabPane>
    ...
    methods: {
        ...
        ok(a_id) {
            let pk = "account.delete.address";
            let id = a_id;
            let token = this.getCookie('_lac_k_');
            let url = appset.domain + "/front/ypc/rt/?" + Date.parse(new Date()) + "&pk=" + pk + "&id=" + id + "&token=" + token;
        ...
        //设为默认收货地址
        defAddress(id, address, mobile, name) {
            let pk = "account.add.modify.address";
            let time = new Date().getTime();
            let url = appset.domain + "/front/ypc/rt/?" + time + "&pk=" + pk + "&address=" + address + "&cel=" + mobile + "&name=" + name + "&def=1" + "&id=" + id;
            fetch(url, { method: "POST", credentials: "include" })
                .then(r => r.json())
                .then(d => {
                    if (d.available == true) {
